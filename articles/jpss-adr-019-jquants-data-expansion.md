...
## 発端 — 3/24の日次処理、Phase D完了直後

「あれ、スクエニが売り候補の1位？」

3月24日の日次処理。Phase D（AIによる銘柄推奨）が完了し、続けて記事提案ステップに進んだとき、Phase 2（テクニカル + ファンダメンタルズの複合スコアリング）の売り候補 Top10 を確認した。スクウェア・エニックス・ホールディングス（9684）が上位に入っていた。

スクエニはゲーム大手だ。たしかに株価は軟調だ。2026年1-2月にGoogleが汎用3Dワールド生成AI「Project Genie」を発表し、ゲームセクター全体が売り込まれた。Unity -24%、Roblox -13%と業界全体が下げる中、スクエニも巻き込まれている。加えて通期の最終利益を31%減益に下方修正しており、テクニカル的に売り候補に入ること自体には一理ある。

しかし営業利益は前年同期比+39%の増益だ。生成AIショックやセクター売りの影響はあるにせよ、好業績企業を空売りするのは本来ガードがかかるはずだった。「なんで営業利益+39%の企業がガードをすり抜けている？」という違和感が先に来た。

続けてTop10全体を見渡すと、「好決算企業が7/10を占めている」という感触があった。数値を確認するために `enriched_indicators` を開くと、全銘柄の `op_profit_yoy_pct`（営業利益の前年同期比）が `None` だった。
...
この言葉は開発者として何度も耳にしてきた。しかし今回のバグは、ユニットテストでは絶対に検出できなかった。なぜなら:

- ユニットテストはモックデータを使う。前年データが揃った状態でテストする
- `fetched_at` フィルタの問題は、時間が経過して初めて現れる（120日後）
- date-range scan の射程問題は、本番運用でDBが「今期データのみ」の状態になって初めて顕在化する

本番環境で時間が経過し、キャッシュが現実の状態になって初めて露呈するバグだった。

このバグに気づいたきっかけは、AIエージェントが記事提案ステップで「好決算企業が売り候補上位に多い」という違和感を報告したことだ。日次処理の文脈で全体を見渡したとき、「スクエニが1位というのはおかしい」という気づきが生まれた。
...
- **「実装済み」は「動いている」ではない** — `fetched_at >= cutoff` フィルタは財務データの性質（不変）を無視した設計だった。本番環境で時間が経過して初めて顕在化した
- **日次運用がバグの検出器になる** — ユニットテストでは再現できないタイムスタンプ依存のバグを、AIエージェントが「結果の妥当性確認」の中で発見した
